...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:4080/api/auth/reset-password', {
        method: 'POST',
  ...
  const getPasswordStrength = () => {
    ...
    if (password.length < 8) return { label: 'Too short', color: colors.red, width: '33%' };
    if (password.length < 12) return { label: 'Good', color: colors.orange, width: '66%' };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            ...
            style={{
              ...
              boxShadow: loading || !token ? 'none' : `0 4px 12px ${colors.orange}40`,
            }}
          >
